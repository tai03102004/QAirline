...
module.exports.index = async (req, res) => {
    ...
    const countProducts = await Airport.countDocuments(find); // Tổng sản phẩm
    const objectPagination = paginationHelper(initPagination, req.query, countProducts);

    // End Pagination

    const airports = await Airport.find(find).sort(sort)
        .limit(objectPagination.limitItems) // Giới hạn 1 trang số sản phẩm hiển thị
        .skip(objectPagination.skip); // Bỏ qua sản phẩm sau
    if (Airport.length > 0 || countProducts == 0) {
        res.render("admin/pages/airports/index", {
            pageTitle: "Danh sách sân bay",
            airports: airports,
